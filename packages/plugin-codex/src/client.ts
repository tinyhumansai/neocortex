...
export class NeocortexMemoryClient {
  ...
  // ---- Mastra-aligned newer endpoints ----
  ...
  async getDocument(params: { document_id: string; namespace?: string }): Promise<any> {
    return this.get(
      `/v1/memory/documents/${encodeURIComponent(params.document_id)}`,
      { namespace: params.namespace },
    );
  }
  ...
  async getIngestionJob(params: { job_id: string }): Promise<any> {
    return this.get(`/v1/memory/ingestion/jobs/${encodeURIComponent(params.job_id)}`, undefined);
  }
