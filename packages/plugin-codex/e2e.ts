...
async function run() {
  ...
  const docId = `codex-doc-${Date.now()}`;
  const batchDocId1 = `codex-batch-doc-1-${Date.now()}`;
  const batchDocId2 = `codex-batch-doc-2-${Date.now()}`;
  ...
  try {
    const insertDocRes = await memory.insertDocument({
      title: "Codex E2E Doc",
      content: "Codex E2E document: preferred drink is coffee.",
      namespace,
      source_type: "doc",
      metadata: { source: "plugin-codex-e2e" },
      priority: "low",
  ...
  try {
    const res = await memory.recordInteractions({
      namespace,
      entity_names: ["Codex-ENTITY-A", "Codex-ENTITY-B"],
      description: "Recorded via plugin-codex-e2e",
  ...
  try {
    const res = await memory.interactMemory({
      namespace,
      entity_names: ["Codex-ENTITY-A"],
      description: "Interacted via plugin-codex-e2e",
      interaction_level: "engage",
      timestamp: undefined,
    });
  ...
  if (workspaceId && agentId) {
    try {
      const res = await memory.syncMemory({
        ...
        files: [
          {
            file_path: "codex-e2e-example.ts",
