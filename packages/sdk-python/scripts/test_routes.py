...
def main() -> int:
    ...
    token = env("TINYHUMANS_TOKEN", "ALPHAHUMAN_TOKEN")
    if not token:
        print("Missing token. Set TINYHUMANS_TOKEN (or ALPHAHUMAN_TOKEN).", file=sys.stderr)
        return 2

    base_url = env("TINYHUMANS_BASE_URL", "ALPHAHUMAN_BASE_URL")
