...
class InteractMemoryTool(Tool):
    """Interact with memory graph (POST /v1/memory/interact)."""
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> dict[str, Any]:
        ...
        if not description:
            return self.create_text_message("Error: description is required.")
        ...
        try:
            ...
            return self.create_text_message(f"Failed to interact memory: {exc}")
