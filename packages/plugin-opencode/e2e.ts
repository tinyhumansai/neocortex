...
async function run() {
  ...
  const docId = `opencode-doc-${Date.now()}`;
  const batchDocId1 = `opencode-batch-doc-1-${Date.now()}`;
  const batchDocId2 = `opencode-batch-doc-2-${Date.now()}`;
  ...
  try {
    const insertDocRes = await memory.insertDocument({
      title: "OpenCode E2E Doc",
      content: "OpenCode E2E document: preferred drink is coffee.",
      namespace,
      source_type: "doc",
      metadata: { source: "plugin-opencode-e2e" },
      priority: "low",
  ...
  try {
    const res = await memory.recordInteractions({
      namespace,
      entity_names: ["OpenCode-ENTITY-A", "OpenCode-ENTITY-B"],
      description: "Recorded via plugin-opencode-e2e",
  ...
  try {
    const res = await memory.interactMemory({
      namespace,
      entity_names: ["OpenCode-ENTITY-A"],
      description: "Interacted via plugin-opencode-e2e",
      interaction_level: "engage",
      timestamp: undefined,
    });
  ...
  if (workspaceId && agentId) {
    try {
      const res = await memory.syncMemory({
        ...
        files: [
          {
            file_path: "opencode-e2e-example.ts",
            content: "E2E syncMemory content (generated).",
            timestamp: Date.now(),
            hash: "opencode-e2e-hash",
          },
        ],
      });
