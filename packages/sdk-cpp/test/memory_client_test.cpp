...
TEST(MemoryClientTest, ConstructorRejectsEmptyToken) {
    EXPECT_THROW(AlphahumanMemoryClient(""), std::invalid_argument);
}
...
TEST(MemoryClientTest, ConstructorAcceptsValidToken) {
    ...
    AlphahumanMemoryClient client("test-token", server.base_url());
    // No throw = success
}
...
TEST(MemoryClientTest, NonOkStatusThrowsAlphahumanError) {
    MockHttpServer server;
    ...
    try {
        ...
        FAIL() << "Expected AlphahumanError";
    } catch (const AlphahumanError& err) {
...
TEST(MemoryClientTest, NonJsonResponseThrowsAlphahumanError) {
    MockHttpServer server;
