...
First Neocortex model release (February 2026). Invoke via the gateway using model id **`mk1-2026-02`**. See the [repository README](../README.md) for base URL and endpoints.
...
Model id **`mk1-2026-02`** supports ingesting memory data points via the gateway’s memory endpoint:

**POST** `https://api.tinyhuman.xyz/memory/ingest`

- **Body**: `{"model": "mk1-2026-02", "data": [<data points>]}`. Each data point is a JSON object (e.g. `{"content": "...", "category": "conversation"}` or as defined by the gateway).
...
The model runs behind the Neocortex/tinyhuman gateway at `https://api.tinyhuman.xyz`. Use **OpenAI-style model routing** by setting `"model": "mk1-2026-02"` in the request body (e.g. `POST /webhook`). Default provider and backing LLM are set in gateway config; “MK1” refers to the gateway + prompt + tool stack for this release.
